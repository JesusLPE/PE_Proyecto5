...
def abrir_admin_en_ventana(parent_root):
    """Abre el panel de administrador en una ventana separada (sin cerrar la ventana actual)."""
    win = tk.Toplevel(parent_root)
    # Si cierran solo la ventana del admin, no se cierra toda la app
    win.protocol("WM_DELETE_WINDOW", win.destroy)
    try:
        # Evita que herede el tamaño anterior muy pequeño
        win.minsize(800, 600)
    except Exception:
        pass
    login_admin(win)
...
def mostrar_inicio(root):
    ...
    btn_login_admin = crear_boton_estilo(opciones_frame, " Abrir Panel de Administrador", 
                                         lambda: abrir_admin_en_ventana(root), 'secondary', 25)
...
def login_admin(root):
    ...
    titulo = crear_label_titulo(main_frame, " Abrir Panel de Administrador", 16)
    titulo.pack(pady=(20, 30))
    titulo.config(bg='#FFFFFF')
